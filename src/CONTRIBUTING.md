...
2. **Implement `render_frame(device, t)`** — return a dict of `{lamp_index: "#rrggbb"}` for each frame. The function is called once per connected device, so your effect automatically works on keyboards, lamps, mice, etc.
