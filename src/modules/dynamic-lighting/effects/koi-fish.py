...
runner = EffectRunner("Koi Fish Pond")
...
def render_frame(device, t):
    ...
    # Lamp devices get a smooth orange-to-blue sweep
    if getattr(device, 'kind', 'Keyboard') != 'Keyboard':
        colors = {}
        for lamp in device.lamps:
            sweep = math.sin(lamp['x'] * math.pi - t * 0.8) * 0.5 + 0.5
            color = lerp(KOI_ORANGE, POND_MID, sweep)
            colors[str(lamp['idx'])] = hex_color(*color)
        return colors

    # Keyboard gets the full koi pond
...
# 87-key TKL layout (for visualization only)
...
_viz_device = types.SimpleNamespace(lamps=lamps)
...
for si, t in enumerate(snapshots):
    frame_colors = render_frame(_viz_device, t)
