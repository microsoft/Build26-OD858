...
public sealed class LightingWindow : IDisposable
{
    ...
    /// <summary>
    /// Hides the companion window to the system tray. The tray icon remains visible
    /// so the user can restore it with a click. Device access is preserved because
    /// the process still owns the LampArray session.
    /// </summary>
    public void HideToTray()
    {
        ...
        {
            _form.BeginInvoke(HideToTrayCore);
            return;
        }
    ...
    private static System.Drawing.Icon CreatePaletteIcon(int size = 16)
    {
        // Draw an artist palette icon at the requested size
        var bmp = new System.Drawing.Bitmap(size, size);
        float s = size / 16f; // scale factor
        using (var g = System.Drawing.Graphics.FromImage(bmp))
        {
            ...
            using var outlinePen = new System.Drawing.Pen(System.Drawing.Color.FromArgb(80, 70, 60), 1.2f * s);
            g.FillEllipse(paletteBrush, 1*s, 2*s, 14*s, 12*s);
            g.DrawEllipse(outlinePen, 1*s, 2*s, 14*s, 12*s);

            // Thumb hole
            using var holeBrush = new System.Drawing.SolidBrush(System.Drawing.Color.FromArgb(60, 55, 50));
            g.FillEllipse(holeBrush, 3*s, 8*s, 3*s, 3*s);
            ...
            g.FillEllipse(red, 5*s, 3*s, 3*s, 3*s);
            g.FillEllipse(blue, 9*s, 3*s, 3*s, 3*s);
            g.FillEllipse(green, 11*s, 6*s, 3*s, 3*s);
            g.FillEllipse(yellow, 8*s, 9*s, 3*s, 3*s);
            g.FillEllipse(purple, 4*s, 7*s, 2.5f*s, 2.5f*s);
        }
