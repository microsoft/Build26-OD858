...
public sealed class CommandHandler
{
    ...
    // -----------------------------------------------------------------------
    // HIDE
    // Minimizes the companion window to the system tray.
    // -----------------------------------------------------------------------
    private string HandleHide()
    {
        _window.HideToTray();
        return "OK hidden";
    }
    ...
    private async Task<(string DeviceId, LampArray Device)?> ResolveTargetDeviceAsync(string? deviceId)
    {
        ...
        // Prefer keyboard over other device types
        var preferred = devices.FirstOrDefault(d => d.IsKeyboard) ?? devices[0];
        var lampArray = await _lampArrayService.GetDeviceAsync(preferred.Id);
        return (preferred.Id, lampArray);
    }
