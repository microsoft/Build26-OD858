...
| `SET_LAMPS_MULTI <json>` | Control LEDs on multiple devices: `{device_id: {index: "#rrggbb"}}` |
| `GET_ALL_LAYOUTS` | Get layouts for all connected devices in one call |
...
**To create your own:** Copy `effects/_template.py` and implement the `render_frame(device, t)` function. Effects automatically render to all connected devices (keyboards, lamps, mice, mousepads, etc.) via the shared `_runner.py` module.
